/* Compact SHA-256, public-domain style. Self-contained so the workload has no
 * link-time dependencies -- it must build identically on ba8 (cross) and z08. */
#ifndef SHA256_H
#define SHA256_H
#include <stdint.h>
#include <string.h>
typedef struct { uint32_t s[8]; uint64_t n; uint8_t b[64]; size_t bl; } sha256_t;
...
static void sha256_update(sha256_t *c,const void *dp,size_t len){
    const uint8_t *d=(const uint8_t*)dp; c->n+=len;
    while(len){ size_t k=64-c->bl; if(k>len)k=len;
        memcpy(c->b+c->bl,d,k); c->bl+=k; d+=k; len-=k;
        if(c->bl==64){ sha256_block(c,c->b); c->bl=0; } }
}
static void sha256_final(sha256_t *c,char out[65]){
    uint64_t bits=c->n*8; uint8_t pad=0x80; sha256_update(c,&pad,1);
    uint8_t z=0; while(c->bl!=56) sha256_update(c,&z,1);
    uint8_t l[8]; for(int i=0;i<8;i++) l[i]=(uint8_t)(bits>>(56-8*i));
    sha256_update(c,l,8);
    for(int i=0;i<8;i++) sprintf(out+i*8,"%08x",c->s[i]);
    out[64]=0;
}
#endif
