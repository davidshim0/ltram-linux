...
#           plateau at the window size, promote_failed climbs, the workload keeps
...
#           promoted by only reading it, then writes every page and checks. Words
...
if [ -n "${PID:-}" ]; then
        ...
        echo "# t in_use promoted promote_failed demoted wr_ok" > "$OUT/t2.curve"
        FLAT=0; LAST=-1
        for t in $(seq 1 150); do
                [ -d /proc/$PID ] || { say "workload exited"; break; }
                ST=$(sudo -n cat $S)
                I=$(awk '/pages_in_use/{print $2}'<<<"$ST"); P=$(awk '/^promoted/{print $2}'<<<"$ST")
                F=$(awk '/promote_failed/{print $2}'<<<"$ST"); D=$(awk '/^demoted/{print $2}'<<<"$ST")
                echo "$((t*10)) $I $P $F $D $(wok)" >> "$OUT/t2.curve"
                [ $((t % 6)) -eq 0 ] && say "  t+$((t*10))s in_use $I promoted $P failed $F demoted $D"
...
if [ -n "${PID:-}" ]; then
        ...
        echo $PID | sudo -n tee $TP >/dev/null; say "attached $PID; waiting for it to be promoted"
        echo "# t in_use lt_pages demoted wr_ok" > "$OUT/t3.curve"
        BEST=0
        for t in $(seq 1 90); do
                [ -d /proc/$PID ] || break
                LT=$(sudo -n $HOME/ltram-inspect $PID $R 2>/dev/null | awk '/^data/{print $4}')
                D=$(awk '/^demoted/{print $2}' <(sudo -n cat $S))
                echo "$((t*10)) $(inuse) ${LT:-0} $D $(wok)" >> "$OUT/t3.curve"
                [ $((t % 3)) -eq 0 ] && say "  t+$((t*10))s  on flash ${LT:-0}/16384  demoted $D"
...
echo "residency after the write (want ~0 -- everything demoted back to DRAM):"
...
elif [ $T3 -eq 3 ];  then echo "TEST 3: INCONCLUSIVE -- never got promoted, so the write proved nothing"
