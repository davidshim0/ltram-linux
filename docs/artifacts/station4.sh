...
set -u
N=${N:-4096}; ITERS=${ITERS:-60}; RUNS=${RUNS:-15}; BATCH=${BATCH:-256}
...
sudo -n $MM --n $N --iters $ITERS --runs 3 --verify --protect-weights --print-ranges --phys \
     > "$OUT/p0.log" 2>&1
...
sudo -n $MM --n $N --iters $ITERS --runs $RUNS --verify --protect-weights --print-ranges --phys --hold $HOLD \
     > "$OUT/p1.log" 2>&1 &
SUDOPID=$!
