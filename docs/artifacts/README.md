...
| `kernel-module-driver.html` | [58cf54b9](https://claude.ai/code/artifact/58cf54b9-bda4-4a73-8b52-33d8c344af7d) | Six pages on where the code lives: why the policy is built into `vmlinuz` while the flash-touching code is a loadable module, what "driver" means concretely here, what the 6,937-line fulltest harness actually is, and the one struct that is the entire interface between them. |
